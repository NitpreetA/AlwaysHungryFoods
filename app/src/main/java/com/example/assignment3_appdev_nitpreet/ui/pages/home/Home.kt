...
import com.example.assignment3_appdev_nitpreet.ui.layout.HungrysScaffold
...
{
    var cartItems = remember { mutableListOf<Food>() }
    //Used to remember the list
    cartItems = rememberMutableStateListOf()

    //Gets the different types of categorises created in the FoodCategories
    val foodsCollections = remember {
        FoodRepo.getFoodCat()
    }
    HungrysScaffold(bottomBar = { Bottom()}, topBar = { TopBar(modifier)}, content ={
        Box(modifier){
            LazyColumn{
                ...
                itemsIndexed(foodsCollections){index,foodCollection->
                    ...
                    Items(foodItems = foodCollection.foods, items =cartItems )
                }
            }
        }
